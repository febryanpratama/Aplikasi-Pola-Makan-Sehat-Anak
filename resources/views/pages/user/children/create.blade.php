<x-app-layout>
          ...
          <div class="max-w-7xl mx-auto sm:px-6 lg:px-8">
               <div class="bg-white dark:bg-gray-800 overflow-hidden shadow-sm sm:rounded-lg">
                    <div class="relative overflow-x-auto shadow-md sm:rounded-lg">
                         ...
                         <form class="w-full mx-auto p-4" action="{{ route('user.children.store') }}" method="POST" enctype="multipart/form-data">
                              @csrf
                              @method('POST')
                              ...
                              <div class="py-2">
                                   ...
                                   <select name="gander" id="gander" class="block mt-1 w-full border-gray-300 dark:border-gray-700 dark:bg-gray-900 dark:text-gray-300 focus:border-indigo-500 dark:focus:border-indigo-600 focus:ring-indigo-500 dark:focus:ring-indigo-600 rounded-md shadow-sm">
                                        ...
                                        <option value="boy">{{ __('Laki-laki') }}</option>
                                        <option value="girl">{{ __('Perempuan') }}</option>
                                   </select>
                              ...
                              <div class="py-2">
                                   ...
                                   <x-text-input id="birhdate" class="block mt-1 w-full" type="date" name="birhdate" :value="old('birhdate')" required autofocus autocomplete="birhdate" placeholder="Tulis Nama Kelompok Makanan" />
                                   <x-input-error :messages="$errors->get('birhdate')" class="mt-2" />
                              </div>
                             
                              {{-- Birth --}}
                              <div class="py-2">
                                   <x-input-label for="birth" :value="__('Usia Sekarang')" />
                                   <x-text-input id="birth" class="block mt-1 w-full" type="text" name="birth"  :value="old('birth')" readonly autocomplete="place" placeholder="Usia Sekarang" />
                              ...
                              <div class="py-2">
                                   ...
                                   <x-text-input id="place" class="block mt-1 w-full" type="text" name="place" :value="old('place')" required autofocus autocomplete="place" placeholder="Tulis Nama Kelompok Makanan" />
                                   <x-input-error :messages="$errors->get('place')" class="mt-2" />
                              </div>
                              ...
                              <div class="py-2">
                                   ...
                                   <x-text-input id="height" class="block mt-1 w-full" type="number" name="height" :value="old('height')" required autofocus autocomplete="height" placeholder="Tulis Nama Kelompok Makanan" />
                              ...
                              <div class="py-2">
                                   ...
                                   <x-text-input id="weight" class="block mt-1 w-full" type="number" name="weight" :value="old('weight')" required autofocus autocomplete="weight" placeholder="Tulis Nama Kelompok Makanan" />
                              ...
                              <div class="py-2">
                                   ...
                                   <textarea name="notes" id="notes" class="block mt-1 w-full border-gray-300 dark:border-gray-700 dark:bg-gray-900 dark:text-gray-300 focus:border-indigo-500 dark:focus:border-indigo-600 focus:ring-indigo-500 dark:focus:ring-indigo-600 rounded-md shadow-sm h-100">{{ old('notes') }}</textarea>
     ...
     <script>
          $(document).ready(function(){
               $('#birhdate').on('change', function(){
     
                    let value = $(this).val(); // Get the value of the birth date

                    // Check if the input value is not empty or null
                    if (!value) {
                    console.log("Input value is empty or null.");
                    } else {
                    console.log("Input value:", value);

                    let date = new Date(value); // Create a new Date object from the birth date
                    ...
                    if (isNaN(date.getTime())) {
                         ...
                         if (date > now) {
                              console.log("Invalid birth date. Date is in the future.");
                         } else {
                              let diff = now - date; // Calculate the difference in milliseconds between the current date and the birth date
                              console.log("Difference in milliseconds:", diff);

                              // let age = Math.floor(diff / 31557600000); // Convert the difference to years (assuming a year has 365.25 days on average)
                              // console.log("Calculated age:", age);

                              // console.log(age); // Output the calculated age to the console

                              const millisecondsInMonth = 30.44 * 24 * 60 * 60 * 1000;

                              // Convert milliseconds to months
                              const months = diff / millisecondsInMonth;

                              // Convert to months to 2 decimal places
                              const age = months.toFixed(2)+' Bulan';


                              $('#birth').val(age); // Set the value of an element with the id 'birth' to the calculated age
                         }
                    }
                    }
